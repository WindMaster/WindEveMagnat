...
namespace WindEveMagnat.UI.DataAccess
{
	public static class MarketItemsHelper
	{
		public static List<TransactionRow> GetMarketItemsRowsForGroup(int groupId)
		{
			...
			var items = Cached.InvTypes.Item.Where(x=> x.Value.MarketGroupId == groupId).Select(y=>y.Value);
		...
		private static void ProcessItemsToTransactionList(List<TransactionRow> result, IEnumerable<InvType> items)
		{
			foreach (var item in items)
			{
				if(item == null)
					continue;
				// 30% waste for t2
				var buildCost = EveMathService.GetBuildCostByJitaPrices(item.Id);
				...
				var groupName = "<none>";
				var jitaPrice = CachedPrices.GetSellPrice(item.Id);
				var vfkPrice = CachedPrices.GetSellPrice(item.Id, MapRegion.Deklein.Id);
